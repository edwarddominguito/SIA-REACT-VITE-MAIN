...
function StatCard({ label, value, icon, helper, tone = "neutral", delta }) {
  return (
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (!requestedStateTab || !validTabSet.has(requestedStateTab)) return;
  ...
  const filteredAppointments = useMemo(() => {
    ...
    return sortedAppointments.filter((a) => {
      ...
      const passStatus =
        ...
        (appointmentStatusFilter === "active" && (status === "pending" || status === "approved" || status === "rescheduled")) ||
        (appointmentStatusFilter === "finished" && (status === "done" || status === "declined" || status === "cancelled")) ||
  ...
  const doneAppointmentsCount = useMemo(
    () => apps.filter((a) => String(a.status || "").toLowerCase() === "done").length,
    [apps]
  );
  ...
  const recentActivity = useMemo(() => {
    ...
    apps.forEach((a) => {
      ...
      const stamp = new Date(a.date ? `${a.date}T${a.time || "00:00"}` : a.createdAt || 0).getTime() || 0;
      feed.push({
        id: `appt-${a.id}`,
        type: "Appointment",
    ...
    meets.forEach((m) => {
      ...
      const stamp = new Date(m.date ? `${m.date}T${m.time || "00:00"}` : m.createdAt || 0).getTime() || 0;
      feed.push({
        id: `meet-${m.id}`,
        type: "Office Meet",
        icon: "bi-building",
        title: m.fullName || m.customer || "Office meet request",
    ...
    trips.forEach((t) => {
      ...
      const stamp = new Date(t.date ? `${t.date}T${t.time || "00:00"}` : t.createdAt || 0).getTime() || 0;
      feed.push({
        id: `trip-${t.id}`,
        type: "Tour",
        icon: "bi-car-front",
  ...
  const saveUserEdits = () => {
    ...
    const nextUsers = users.map((u) => {
      ...
      if (role === "agent") {
        next.availabilityStatus = availabilityStatus;
      } else {
  ...
  const assignAppointmentAgent = (appointmentId, assignedAgentUsername) => {
    ...
    >
          ...
          <>
            ...
            <section className="agent-stats-grid admin-stats-grid">
              ...
              <StatCard
                ...
                delta={`${pendingMeets.length} pending office meets`}
              />
              <StatCard
            ...
            <section className="admin-dashboard-insights-grid">
              <article className="agent-panel admin-analytics-card">
                ...
                <MiniBarChart data={monthlyActivityData} />
                <div className="admin-distribution-grid">
                  ...
                  <div className="admin-distribution-row">
                    <span>Approved</span>
                    <strong>{appointmentDistribution.approved}</strong>
                  </div>
                  <div className="admin-distribution-row">
                  ...
                  <div className="admin-distribution-row">
                    <span>Done</span>
                    <strong>{appointmentDistribution.done}</strong>
                  </div>
                </div>
              </article>
        ...
        {tab === "calendar" && (
          <DashboardCalendar
            ...
            subtitle="Appointments, office meets, and tours in one monthly view."
        ...
        {tab === "users" && (
          <section className="agent-panel admin-users-management">
            <div className="admin-users-hero">
              ...
              <p>Manage all users in one place. Search profiles, monitor account health, and create new agent accounts quickly.</p>
            </div>
                ...
                <article className="admin-users-add-card admin-user-edit-card">
                  ...
                  >
                    ...
                    <div className="admin-create-user-actions">
                      <button type="button" className="btn btn-outline-dark" onClick={cancelUserEditor}>
                        Cancel
                      </button>
                                ...
                                onClick={() => {
                                  feedback.askConfirm({
                                    ...
                                    onConfirm: () => {
                                      ...
                                      const retainedApps = apps
                                        .filter((a) => a.agent !== u.username && a.customer !== u.username)
                                        .map((a) => (a.assignedAgent === u.username ? { ...a, assignedAgent: "", assignedAt: "", assignedByAdmin: "" } : a));
                                      saveApps(retainedApps);
                ...
                <article key={p.id} className="agent-property-card">
                  ...
                  <div className="agent-property-body">
                    ...
                    <div className="agent-property-meta">
                      <span><i className="bi bi-person-badge"></i> {formatAgentIdentity(p.agent)}</span>
